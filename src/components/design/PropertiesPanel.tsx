...
const PropertiesPanel = ({
	...
	const getCurrentColor = () => {
		if (selectedElement instanceof fabric.Group && isAnchor) {
			...
			return textObject ? (textObject.get("fill") as string) || "#000000" : "#000000";
		}
		return (
	...
	const currentStrokeWidth = (selectedElement.get("strokeWidth") as number) ?? 1;
	const currentBorderColor = (selectedElement.get("stroke") as string) || "#000000";
	...
	const getFontProperty = (property: string, defaultValue: any) => {
		if (selectedElement instanceof fabric.Group && isAnchor) {
			...
			const textObject = selectedElement
				.getObjects()
				.find((obj) => obj instanceof fabric.Textbox);
			return textObject ? textObject.get(property) || defaultValue : defaultValue;
		}
		...
		<>
			<div className="bg-designcanvas_background border border-gray-300 w-[854px] h-[60px] rounded-full px-10 mb-4 shadow-sm mx-auto flex items-center justify-between">
				...
				<div className="flex items-center gap-4">
					...
					{isImage && (
						<div className="flex items-center gap-3">
							<span className="text-base font-medium text-white">Image selected</span>
						</div>
					)}
					...
					{(isLine ||
						(selectedElement.get("stroke") && !isText && !isImage && !isQRanchor)) && (
						<div className="flex items-center gap-3">
				...
				{!isQRanchor && (
					<div className="flex items-center gap-2">
						...
						<button
							...
							className="px-5 py-2 h-10 text-sm bg-red-700 text-white rounded-full hover:bg-red-600 hover:border-red-600 hover:shadow-md active:bg-red-700 active:border-red-700 active:scale-95 transition-all duration-150"
						>
